let form = document.querySelector(".reading-avg");
        ...
        function formatTime(time) {
            ...
            if (inMinutes === 0) {
                return `00:${leadingZeros(inSeconds)} (${inSeconds} ${inSeconds > 1 ? "Seconds" : "Second"})`;
            } else if (inHours === 0) {
                return `${leadingZeros(inMinutes)}:${leadingZeros(inSeconds)} (${inMinutes} ${inMinutes > 1 ? "Minutes" : "Minute"} ${inSeconds} ${inSeconds > 1 ? "Seconds" : "Second"})`;
            } else {
                return `${inHours}:${leadingZeros(inMinutes)}:${leadingZeros(inSeconds)} (${inHours} ${inHours > 1 ? "Hours" : "Hour"} ${inMinutes} ${inMinutes > 1 ? "Minutes" : "Minute"} ${inSeconds} ${inSeconds > 1 ? "Seconds" : "Second"})`;
            }
        }
        ...
        function showDetails(formatTime, wordLength, lettersLength) {
            ...
            <div class="info-box details">
                <ul>
                    <li><strong>Words Count:</strong> ${wordLength} ${wordLength > 1 ? "Words" : "Word"}</li>
                    <li><strong>Letters Count:</strong> ${lettersLength} ${lettersLength > 1 ? "Letters" : "Letter"}</li>
                </ul>
            </div>
            `;
        }
        ...
        function calcReadingAverage(timeUnitInMinutes = 1) {
            ...
            let lettersLength = textarea.value.trim().replace(/\s+/g, "").split("").length;
